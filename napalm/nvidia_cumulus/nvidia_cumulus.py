...
class NvidiaCumulusDriver(NetworkDriver):
    def __init__(self, hostname, username, password, timeout=60, optional_args=None):
        ...
        self.device = None
        self.platform = "nvidia_cumulus"

    def open(self):
        try:
            from netmiko import ConnectHandler
            self.device = ConnectHandler(
                device_type='linux',  # Or a more specific Cumulus type if Netmiko has one
                host=self.hostname,
            ...
            raise ConnectionException(f"Could not connect to {self.hostname}: {e}")
    ...
    def get_facts(self):
        ...
        facts = {}
        try:
            # Example for Cumulus Linux using 'uname -a' and 'cat /etc/os-release'
            # You'd need to adapt this for MLNX-OS
            output_uname = self.device.send_command("uname -a")
            output_os_release = self.device.send_command("cat /etc/os-release")
            output_hostname = self.device.send_command("hostname -f")

            # Parse uname output for OS version and model (this is a simplified example)
            match_os = re.search(r'Linux ([\w.-]+)', output_uname)
            if match_os:
                facts['os_version'] = match_os.group(1)
            
            # Parse /etc/os-release for vendor
            match_vendor = re.search(r'ID_LIKE=(.+)', output_os_release)
            if match_vendor and "debian" in match_vendor.group(1): # Cumulus is Debian-based
                facts['vendor'] = 'NVIDIA (Cumulus Networks)'
            
            # Extract hostname and FQDN
            facts['hostname'] = output_hostname.strip().split('.')[0]
            facts['fqdn'] = output_hostname.strip()

            # For model and serial number, you'd typically need specific Cumulus/NVIDIA commands
            # Example (hypothetical):
            # output_model = self.device.send_command("nv-cli show system model")
            # facts['model'] = output_model.strip()
            facts['model'] = self._get_system_entity("Part Number")

            # output_serial = self.device.send_command("nv-cli show system serial-number")
            # facts['serial_number'] = output_serial.strip()
            facts['serial_number'] = self._get_system_entity("Serial Number")
            
            # Uptime (example for Linux systems)
            output_uptime = self.device.send_command("cat /proc/uptime")
            uptime_seconds = float(output_uptime.split()[0])
            facts['uptime'] = int(uptime_seconds)


        except Exception as e:
            raise ValueError(f"Error getting facts from {self.hostname}: {e}")

        # Add placeholders for other facts if not available
        facts.setdefault('model', 'Unknown')
        facts.setdefault('serial_number', 'Unknown')

        return facts

    def _get_system_entity(self, sys_type):
        sys_entities = {}
        command = "decode-syseeprom"
        output = self.device.send_command(command)

        in_tlv_section = False
        for line in output.splitlines():
            # Look for the header line to start parsing TLVs
            if "---- --- -----" in line:
                in_tlv_section = True
                continue

            if in_tlv_section and line.strip(): # Ensure line is not empty
                try:
                    tlv_name_raw = line[0:20].strip() # Adjust width as needed for longest names
                    code_raw = line[20:25].strip()
                    len_raw = line[25:29].strip()
                    value_raw = line[29:].strip() # Value goes to the end of the line

                    tlv_name = ' '.join(tlv_name_raw.split()) # Normalize internal spaces

                    if tlv_name: # Only add if we successfully parsed a name
                        sys_entities[tlv_name] = value_raw
                except IndexError:
                    # This line might not conform to the expected TLV format (e.g., TlvInfo Header)
                    continue

        if sys_type.lower() == "all":
            return sys_entities
        else:
            return sys_entities.get(sys_type)
